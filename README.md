...
A simple, one-command WordPress installer using Docker. Just pull the image, run the installer script, and have WordPress running in minutes.
...
```bash
git clone https://github.com/sfowooza/wordpress-docker-installer.git
cd wordpress-docker-installer
```
...
1. **Port** - The port to run WordPress on (default: 8080)
   - Automatically checks if port is available
   - Notifies you if port is in use and what process is using it

2. **Site Title** - Your WordPress site name

3. **Admin Username** - Your WordPress admin username

4. **Admin Password** - Your WordPress admin password (min 8 characters)

5. **Admin Email** - Your WordPress admin email

6. **WordPress URL** - Your site URL (default: localhost:PORT)

7. **Database** - Press Enter to use auto-generated secure credentials
...
- GitHub Issues: https://github.com/sfowooza/wordpress-docker-installer/issues
